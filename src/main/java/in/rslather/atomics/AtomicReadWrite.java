...
public class AtomicReadWrite<V> {
	 ...
	 * @throws Exception
	 */
	public <T> T callWithOptimisticLock(final int maxRetries, final Callable<T> callable)
			throws Exception {
	 ...
	 * @throws Exception
	 */
	public <T> T callAndUpdate(final Callable<T> readAndCall,
			final Predicate<T> isUpdatable,
		...
		try {
			while (true) {
				...
				if (isUpdatable.test(t)) {
					final long writeTryStamp = this.stampedLock
	 ...
	 * @throws Exception
	 */
	public <T> T tryCallUpdate(final Callable<T> tryCall, final Supplier<V> supplier)
			throws Exception {
		long stamp = this.stampedLock.readLock();
		try {
			while (true) {
	...
	public static <V> AtomicReadWrite<V>[] buildAsArray(final int capacity) {
		return (AtomicReadWrite<V>[]) Array
				.newInstance(new AtomicReadWrite<V>().getClass(), capacity);
	}
